...
try:
	...
	for i in range(9):
		cursor.execute(
			'INSERT INTO types(type, ped_priority, bike_priority, vehicle_priority, comm_activity, max_speed_limit)' +
			' VALUES("' + types[i] + '", ' + str(pedP[i]) + ', ' + str(bikeP[i]) + ', ' + str(vehicleP[i]) + ', ' + str(
				commA[i]) + ', ' + str(speedLimit[i]) + ')')
	mydb.commit()
except:
